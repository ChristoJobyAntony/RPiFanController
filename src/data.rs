...
impl FanSpeed {
    ...
    pub fn new(min_duty_cycle:f32, min_threshold:f32, max_threshold:f32) -> FanSpeed{
        ...
        if min_threshold > max_threshold {panic!("Incorrectly configure FanSpeed !, Minimum fan threshold  greater than maximum threshold ")}
        ...
        let mut f = FanSpeed {steps, min_duty_cycle, min_threshold, max_threshold};
    ...
    pub fn push(&mut self, s : SpeedStep) {
        if s.temperature > self.min_threshold || s.temperature > self.max_threshold {panic!("Temperature is out of fan threshold range !")}
        if s.fan_speed < self.min_duty_cycle || s.fan_speed > 100f32 {panic!("Fan Duty Cycle is out of range !")}
    ...
    pub fn get_speed(&self, temp: f32) -> f32 {
        let mut speed = 0f32;
        if temp < self.min_threshold {speed = 0f32}
        else if temp >= self.max_threshold {speed = 1.0f32}
        else {
...
impl SpeedStep {
    pub fn new (temperature:f32, fan_speed:f32) -> SpeedStep{
        if fan_speed > 1f32 || fan_speed < 0f32 { panic!("FanSpeed should ber between 0-1")}
